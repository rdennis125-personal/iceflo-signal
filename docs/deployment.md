...
The initial ingest path supports a user OAuth configuration for a client-owned Google Drive data root. Local runs use environment variables to point at the root folder ID, OAuth client secret file, and token file. Cloud deployment should replace local token files with managed secrets and mount or inject the values at runtime.
...
- Terraform-managed Artifact Registry, Secret Manager placeholders, Cloud Run runtime service account, Cloud Run Job, and client-root storage prefixes.
...
3. Secret Manager secret versions for the Mindful Oregon Drive root folder ID, OAuth client secret JSON, and OAuth token JSON.
